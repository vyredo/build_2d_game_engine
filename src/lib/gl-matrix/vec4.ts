...
export function create() {
  var out = new glMatrix.ARRAY_TYPE(4) as vec4;
...
export function min(out: vec4, a: vec4, b: vec4) {
...
export function max(out: vec4, a: vec4, b: vec4) {
...
export function scaleAndAdd(out: vec4, a: vec4, b: vec4, scale: number) {
  out[0] = a[0] + b[0] * scale;
  out[1] = a[1] + b[1] * scale;
  out[2] = a[2] + b[2] * scale;
  out[3] = a[3] + b[3] * scale;
...
export function negate(out: vec4, a: vec4) {
  out[0] = -a[0];
  out[1] = -a[1];
  out[2] = -a[2];
  out[3] = -a[3];
...
export function cross(out: vec4, u: vec4, v: vec4, w: vec4) {
...
export function random(out: vec4, scale: number) {
...
export var forEach = (function () {
  var vec = create();
  return function (a: number[], stride: number, offset: number, count: number, fn: Function, arg: {}) {
    var i, l;
